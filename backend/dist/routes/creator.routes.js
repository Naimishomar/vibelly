...
const CreatorPost_1 = __importDefault(require("../models/CreatorPost"));
...
const ensureProfile = async (userId) => {
    let profile = await CreatorProfile_1.default.findOne({ user: userId });
    if (!profile) {
        profile = await CreatorProfile_1.default.create({ user: userId });
    }
    return profile;
};
const serializeProfile = async (profile, viewerId) => {
    const user = await User_1.default.findById(profile.user).select('_id name username profileImage premiumStatus role isBanned');
    const [followerCount, followingCount, isFollowing] = await Promise.all([
        Follow_1.default.countDocuments({ following: profile.user }),
        Follow_1.default.countDocuments({ follower: profile.user }),
        viewerId ? Follow_1.default.exists({ follower: viewerId, following: profile.user }) : Promise.resolve(null),
    ]);
    return {
        _id: profile._id,
        user,
...
router.get('/:userId', async (req, res) => {
    try {
        const targetUser = await User_1.default.findById(req.params.userId);
        if (!targetUser)
...
router.get('/me/profile', auth_middleware_1.requireAuth, async (req, res) => {
    try {
        console.log('[Creator me] User ID:', req.user.id, 'Auth header:', req.headers.authorization?.substring(0, 20) + '...');
        const profile = await ensureProfile(req.user.id);
        console.log('[Creator me] Profile created/found:', profile._id);
        const data = await serializeProfile(profile, req.user.id);
        res.json({ profile: data });
    }
    catch (error) {
...
router.put('/me/profile', auth_middleware_1.requireAuth, async (req, res) => {
    try {
        const { bio, coverImage, subscriptionPrice } = req.body;
        const profile = await ensureProfile(req.user.id);
        if (typeof bio === 'string')
            profile.bio = bio.slice(0, 500);
...
router.post('/me/gallery', auth_middleware_1.requireAuth, async (req, res) => {
    try {
        const { url } = req.body;
...
router.delete('/me/gallery', auth_middleware_1.requireAuth, async (req, res) => {
    try {
        const { url } = req.body;
        const profile = await ensureProfile(req.user.id);
        profile.galleryPhotos = profile.galleryPhotos.filter((p) => p !== url);
        await profile.save();
        res.json({ success: true, galleryPhotos: profile.galleryPhotos });
    }
    catch (error) {
...
router.post('/me/verify', auth_middleware_1.requireAuth, async (req, res) => {
    try {
...
router.get('/streams/:userId', async (req, res) => {
    try {
        const rooms = (0, liveRooms_1.getAllLiveRooms)().filter((r) => r.creatorUserId === String(req.params.userId || ''));
        res.json({
            streams: rooms.map((r) => ({
                roomCode: r.roomCode,
                title: r.title,
...
// Follow a creator
router.post('/follow/:userId', auth_middleware_1.requireAuth, async (req, res) => {
    try {
        const follower = req.user.id;
        const following = String(req.params.userId || '');
        if (follower === following)
            return res.status(400).json({ error: 'You cannot follow yourself' });
        const target = await User_1.default.findById(following);
        if (!target)
            return res.status(404).json({ error: 'User not found' });
        const existing = await Follow_1.default.findOne({ follower, following });
        if (!existing) {
            await Follow_1.default.create({ follower, following });
        }
        const followerCount = await Follow_1.default.countDocuments({ following });
...
// Unfollow a creator
router.delete('/follow/:userId', auth_middleware_1.requireAuth, async (req, res) => {
    try {
        const follower = req.user.id;
        const following = String(req.params.userId || '');
        await Follow_1.default.deleteOne({ follower, following });
        const followerCount = await Follow_1.default.countDocuments({ following });
...
router.get('/posts/:userId', async (req, res) => {
    try {
        ...
        const posts = await CreatorPost_1.default.find({ creator: String(req.params.userId || '') })
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(limit)
            ...
            .lean();
        const total = await CreatorPost_1.default.countDocuments({ creator: String(req.params.userId || '') });
...
router.post('/posts', auth_middleware_1.requireAuth, async (req, res) => {
    try {
        ...
        const post = await CreatorPost_1.default.create({
            creator: req.user.id,
            images,
            imageKeys: imageKeys || [],
...
router.delete('/posts/:postId', auth_middleware_1.requireAuth, async (req, res) => {
    try {
        const post = await CreatorPost_1.default.findById(String(req.params.postId || ''));
        if (!post)
        ...
        await Promise.all((post.imageKeys || []).map((key) => (0, r2_service_1.deleteFromR2)(key)));
...
router.post('/posts/:postId/like', auth_middleware_1.requireAuth, async (req, res) => {
    try {
        const post = await CreatorPost_1.default.findById(String(req.params.postId || ''));
        if (!post)
            return res.status(404).json({ error: 'Post not found' });
...
router.post('/posts/:postId/comment', auth_middleware_1.requireAuth, async (req, res) => {
    try {
        const { text } = req.body;
        ...
        if (!post)
            return res.status(404).json({ error: 'Post not found' });
        post.comments.push({ user: req.user.id, text: text.slice(0, 300) });
        await post.save();
...
router.delete('/posts/:postId/comment/:commentId', auth_middleware_1.requireAuth, async (req, res) => {
    try {
        const post = await CreatorPost_1.default.findById(String(req.params.postId || ''));
        if (!post)
            return res.status(404).json({ error: 'Post not found' });
        const userId = req.user.id;
        const comment = post.comments.find((c) => String(c._id) === String(req.params.commentId || ''));
        if (!comment)
        ...
        post.comments = post.comments.filter((c) => String(c._id) !== String(req.params.commentId || ''));
        await post.save();
        res.json({ success: true });
    }
    catch (error) {
