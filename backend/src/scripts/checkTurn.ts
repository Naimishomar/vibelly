...
function allocateRequest(): Buffer {
  ...
  return new Promise((resolve) => {
    ...
    socket.on('message', (msg) => {
      ...
      while (offset < end) {
        ...
        if (type === ATTR_ERROR_CODE) code = value.readUInt8(2) * 100 + value.readUInt8(3);
        if (type === ATTR_REALM) realm = value.toString();
